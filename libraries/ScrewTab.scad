...
module screw_tab(head_d, th, shaft_d=0, head_a=0, padding=0, cone_l=0)
{
    ScrewHeadDiamer=head_d;
    TabScrewHeadPadding=(padding <= 0 ? head_d : padding);
    TabWidth=ScrewHeadDiamer+TabScrewHeadPadding;
    ScrewConeHeight=(cone_l <= 0 ? head_d/2 : cone_l); 
    ScrewShaft_d=(shaft_d <= 0 ? head_d/2 : shaft_d);
    ScrewHeadAngle=(head_a <= 0 ? 45 : head_a);
    TabThickness=th;
    Foo=0.1; //Fudge factor to ensure complete overlap of difference
    difference() {
        union(){
        translate([TabWidth/2,0,0]) 
            cylinder (h=TabThickness, r=TabWidth/2, center = false, $fn=100);
        translate ([0,-TabWidth/2,0]) 
            cube ([TabWidth/2,TabWidth,TabThickness], center = false);
        }
        translate ([-TabWidth/2,-TabWidth/2,-Foo]) 
            cube ([TabWidth/2,TabWidth,TabThickness+Foo*2], center = false);
        translate ([TabWidth/2,0,TabThickness]) 
            rotate([0,180,0])
                screw_hole([ScrewHeadDiamer, ScrewConeHeight, ScrewHeadAngle], ScrewShaft_d, TabThickness);
    }
}
